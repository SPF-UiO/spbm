...
class f60:
    ...
    def paragraf(self, t, par_bredde = 80):
        """Bryter teksten med harde linjeskift p en gitt bredde, 80 tegn hvis ikke annet er oppgitt"""
        linjer = t.replace('\r\n', '\n').split('\n')
        ret = []
        for l in linjer:
            i = par_bredde
    ...
    def fyll(self): #, firma, faktura):
        ...
        f = self.firma
        firmainfo.textLines("""{kontaktperson}
{addresse}
{postnummer} {poststed}
Telefon: {telefon}
Bankkonto: {kontonummer}
Org.nr: {organisasjonsnummer}
Epost: {epost}""".format(**f).split('\n'))
        ...
        #print(repr(self.kunde))
        #a = self.kunde.get('addresse')
        #kunde.textLines("Kunde# {0:03d}\n{1:s}".format(self.kunde['nr'], str(a)));
        #kunde.textLines(split("Kunde# %03i\n%s" % (self.kunde['nr'], self.kunde['adresse']), '\n'))
        kunde.textLines("Kunde lol")
...
Side: %i av %i
        ...
        if type(self.ordrelinje) in (types.FunctionType, types.MethodType):
            ...
            for vare in self.ordrelinje:
                ...
                self.canvas.drawString(tekstX, Y, "%s" % (vare[0]))
                self.canvas.drawRightString(bruttoX, Y, self._kr(brutto))
                self.canvas.drawRightString(mvaX, Y, self._kr(mva))
                self.canvas.drawRightString(prisX, Y, self._kr(pris))
                Y -= 3*mm
